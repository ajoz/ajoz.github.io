...
`Minecraft` is the best example of an indie game success, there are `PC`, `XBox`, `Android` versions available, you can find one even for `Raspberry Pi`. When I last read about this game it had almost 10 milion copies sold! Can you imagine? For a game started as a small project it's a hell of a success.
...
6. Moai - http://getmoai.com/ - Lua - Free - It's an open source project, using it is completly free, but to get full comfort while using the SDK you need to buy a dedicated IDE, which is around 40 usd. Moai has a very rich API but in many places having an outdated documentation or lacking it completly. Many things that are normaly hidden by other frameworks here need to be handled by the programmer.
...
Another aspect is graphics and sound. For each paid tool there are tons of better or worse free alternatives, for example good old Photoshop can be replaced with Gimp. If you need to do some sound manipulation you can relay of free Audacity. It's possible to cover those apsects of game making without additional costs. Unfortunately it's the otherway around in terms of testing and releasing into the market. For me at least it's hard to imagine to develop a game without at least one device. Android device emulator although often capable to run your game in some cases might fail (if your game relies on OpenGL) or just be so slow that it's not possible to test reasonably.

Then there is the market. In case of Google Play we need to pay a one time fee that's around 25 USD when we are creating a developer account. Most of the new Indie devs are waiting with this until they finish their first game. Google is explaining that this fee was created to increase the quality of submitted apps. I believe it was there to lower the amount of cracked apps getting back to the market (which is a whole different subject altogether). GetJar which is a direct competitor to Google Play doesn't have such fees.
